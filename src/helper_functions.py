            ...
            for param in disk:
                if "children" in param:
                    for partition in disk["children"]:
                        partitions.append(f"/dev/{partition["name"]}")
...
def set_swap_configuration(swap_data):
    if swap_data["enabled"]:
        swap_size = swap_data["size"]
        swap_path = swap_data["path"]
        swap_priority = swap_data["priority"]
        zswap_enabled = swap_data["zswap_enabled"]
        zswap_algorithm = swap_data["zswap_algorithm"]
        if swap_data["type"] == "file":
            remove_swapfile()
            generate_swapfile(swap_size)
            swap_path = "/swap/swapfile"
        disable_swap()
        format_mkswap(swap_path)
        backup_fstab()
        clean_swaps_from_fstab()
        add_swap_to_fstab(swap_path, swap_priority)
        enable_swaps()
...
def backup_fstab():
    ...
    backup_path = Path("/etc/fstab.bak")
    if backup_path.exists():
        try:
            shutil.copy2("/etc/fstab", "/etc/fstab.bak") # copy2 function copies with metadata so its just better

        except PermissionError:
            print("Backup fstab failed. Permission error")
        
        except Exception as e:
            print(f"Error while backup fstab: {e}")
    
    else:
        print("There is already a backup taken before")

def read_swap_from_fstab():
    path = None
    ...
    try:
        ...
        for partition in fstab:
            if "swap" in partition:
                path = partition.split()[0]
    
    except PermissionError as e:
        print("permission error read swap fstab")
    
    return path

def clean_swaps_from_fstab():
    fstab_lines_cleaned = []
    try:
        with open("/etc/fstab", "r") as f:
            fstab = f.read().split("\n")
        ...
        for line in fstab:
            if "swap" not in line:
                fstab_lines_cleaned.append(line)

        fstab_new = "\n".join(fstab_lines_cleaned)

        with open("/etc/fstab", "w", encoding="UTF-8") as f:
            f.write(fstab_new)
    
    except PermissionError:
        print("Error while clearing swaps from fstab: Permission error")
    
    except Exception as e:
        print(f"Error while clearing swaps from fstab: {e}")
    
def add_swap_to_fstab(path, priority):
    swap = f"\n{path} none    swap    defaults,pri={priority} 0 0 # Generated by pardus-swap-manager"
    ...
    try:
        ...
        print("Error while adding swap to fstab: Permission error")
    
    except Exception as e:
        print(f"Error while adding swap to fstab: {e}")

def disable_swap():
    path = read_swap_from_fstab()
    if path != None:
        try: # disable swap
            command_disable_swaps = subprocess.run(
                ["swapoff", path],
                check=True,
                stdout=subprocess.PIPE,
                stderr=subprocess.PIPE,
            ...
            print(f"Error while disabling swaps")
    
    else:
        print("no swap in fstab")

def enable_swaps():
    try: # enable all swaps
        command_enable_swaps = subprocess.run(
            ["swapon", "-a"],
            check=True,
            stdout=subprocess.PIPE,
            stderr=subprocess.PIPE,
        ...
        print("Enabled all swaps")
    except subprocess.CalledProcessError as e:
        print(f"Error while enabling swaps")
    
def format_mkswap(path):
    try:
        if "/dev" in path:
            fstype = get_partition_fstype(path)
            if fstype == "swap": 
                return

        command_mkswap = subprocess.run(
            ["mkswap", path],
            check=True,
            stdout=subprocess.PIPE,
            stderr=subprocess.PIPE,
        ...
        print("Partition formatted as swap")
    except subprocess.CalledProcessError as e:
        print(f"Error while formatting partition: {e}")

def generate_swapfile(size):
    folder = Path("/swap/")
    folder.mkdir()
    file = folder / "swapfile"

    try:
        create_swapfile = subprocess.run(
            ["fallocate", "-l", f"{int(size * 1024)}M", file],
            capture_output=True,
            text=True,
            check=True,
        )

        print(f"swap file generated. size: {size}")

    except Exception as e:
        print(f"Error while creating swapfile: {e}")
...
def get_partition_fstype(partition):
    fstype = None
    try:
        get_fstype = subprocess.run(
            ["lsblk", "-J", "-o", "FSTYPE", partition],
            capture_output=True,
            text=True,
            check=True,
        )

        data = json.loads(get_fstype.stdout)
        bdev = data.get("blockdevices", [])[0]
        fstype = bdev["fstype"]

    except Exception as e:
        print(f"Error while getting fstype: {e}")
